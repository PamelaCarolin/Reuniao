...
db.serialize(() => {
    ...
    db.run(`ALTER TABLE meetings ADD COLUMN room TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
            console.error('Erro ao adicionar coluna "room":', err);
        }
    });

    // Adicionar coluna 'client' se ela não existir
    db.run(`ALTER TABLE meetings ADD COLUMN client TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
            console.error('Erro ao adicionar coluna "client":', err);
        }
    });
});
