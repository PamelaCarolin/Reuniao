        ...
        .then(data => {
            ...
            if (data.length > 0) {
                ...
                data.forEach(reuniao => {
                    ...
                    const status = determineStatus(reuniao.date, reuniao.time); // Define o status com base na data e hora
...
// Função para determinar o status da reunião (Concluída ou Agendada)
function determineStatus(date, time) {
    const meetingDateTime = new Date(`${date}T${time}`);
    return meetingDateTime < new Date() ? 'Concluída' : 'Agendada';
}
...
function downloadHistoricoPDF() {
    ...
    fetch(`/consultar-historico?${params.toString()}`)
        ...
        .then(data => {
            ...
            data.forEach(reuniao => {
                const formattedDate = formatDate(reuniao.date);
                const formattedTime = formatTime(reuniao.time);
                const status = determineStatus(reuniao.date, reuniao.time);
